...
- When installing, refreshing, or reviewing `Facepunch/sbox-public`, route through `.agents/sbox/sbox-public-source-agent.md`, keep the project-local checkout under `tools/sbox-public`, run `Bootstrap.bat` after updates, preserve dirty sibling engine checkouts, verify MCP manifest/build/live status, and run `scripts/agents/run_agent_checks.ps1 -Suite sbox-public -ShowInfo`.
