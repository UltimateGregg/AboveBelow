...
HEADER
{
	Description = "A simple fog post processing shader";
}
...
PS
{
	...
	float4 g_vColor < Attribute("Color"); Default4(0.05, 0.09, 0.18, 1.00); >;
	float g_flIntensity < Attribute("Intensity"); Default1(0.5); >;
	float g_flOpacity < Attribute("Opacity"); Default1(0.5); >;
	...
	{
		float3 sceneColor = g_tColorBuffer.Sample(g_sAniso, CalculateViewportUv(MapSceneColorCoords(i.vPositionSs.xy, float2(0,0)))).rgb;

        float linearDepth = Depth::GetLinear(i.vPositionSs.xy);

        float3 sunDirectionWs = -g_DirectionalLightDirection.xyz;

        float sunHeight = saturate(dot(sunDirectionWs, float3(0, 0, 1)));

        if (sunHeight < 0.025)
            sunHeight = 0.025;

        float fogAmount = 1 - exp(linearDepth * -0.0254 * g_flIntensity);
        fogAmount *= g_flOpacity;

        float3 finalColor = saturate(lerp(sceneColor, g_vColor.rgb * sunHeight, fogAmount));
