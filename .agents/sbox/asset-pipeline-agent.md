...
- For solid, non-deforming props, default to `"collision": { "mode": "render_mesh" }`. Primitive collision is an exception only when the brief documents why a mesh collider is wrong, and its dimensions must be derived from exported source-unit mesh bounds, not Blender meter values.
- Treat `physics_shapes` as legacy. If a config still uses primitives, run `model_collision_scale_audit.ps1 -ShowInfo` and verify the primitive union matches the render mesh bounds within tolerance.
- Place solid props through their prefab with a static `ModelCollider`; do not place by dragging the raw `.vmdl`, which can create a dynamic Prop that falls in play mode.
...
- Asset Definition-of-Done for placeable props includes editor proof: place the prefab in `main.scene` or a throwaway test scene, enter play through the S&Box MCP, confirm it renders, stays put, and blocks/traverses as intended, then record `ModelCollider.LocalBounds` approximately matching `ModelRenderer.LocalBounds`.
...
Report config errors, missing source blends, missing material remaps, target path drift, `.vmdl` remap source drift, and render/collision bounds mismatches. Separate pipeline validation from visual/editor validation; a Blender preview is not proof that S&Box compiled and displayed the intended material, nor that the prefab survives play mode.
