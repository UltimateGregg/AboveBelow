...
6. Record `hasUnsavedChanges` from `control_plane_status` / `editor_scene_info`. If the editor was already dirty before your change, treat that state as user-owned: do not call `editor_save_scene`, `scene_open`, reload commands, or any operation likely to raise a save prompt unless the user explicitly approves that path.
