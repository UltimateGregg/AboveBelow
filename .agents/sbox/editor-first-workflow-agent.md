...
- For newly imported or changed solid props, place the prefab, not the raw `.vmdl`, in `main.scene` or a throwaway test scene. Enter play and confirm the prefab renders, stays put, and has working collision. Capture `ModelCollider.LocalBounds` and `ModelRenderer.LocalBounds` evidence; they should be non-zero and approximately equal.
