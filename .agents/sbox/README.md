...
| Facepunch/sbox-public source install or refresh | `sbox-public-source-agent.md` | `powershell -ExecutionPolicy Bypass -File scripts/agents/sbox_public_source_audit.ps1 -Root . -RequireLatest -ShowInfo` |
...
- When the user links `Facepunch/sbox-public`, use `sbox-public-source-agent.md`: keep the project-local clone under `tools/sbox-public`, run `Bootstrap.bat`, preserve dirty sibling engine checkouts, and prove MCP manifest/build/live status with `sbox_public_source_audit.ps1`.
