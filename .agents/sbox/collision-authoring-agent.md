...
- Building collision is evaluated at the building root. A selected `Model_Visual` child can be renderer-only when sibling `Collision_*` children under the house/building root own the blocking shape.
