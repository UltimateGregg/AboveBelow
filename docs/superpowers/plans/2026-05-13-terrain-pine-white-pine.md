...
**Goal:** Replace the existing simple `terrain_pine` model with a tall white-pine-style environment asset.

**Architecture:** Keep existing scene references stable by preserving `models/terrain_pine.vmdl`. Update the Blender generator as the source of truth, add a dedicated pipeline config for repeatable pine-only export, and validate through the asset-production tools.
...
- Create `scripts/terrain_pine_asset_pipeline.json`: export `TerrainPine_Root` from the shared terrain blend to the existing pine FBX/VMDL.
- Modify generated assets: `environment_model.blend/terrain_assets.blend`, `Assets/models/terrain_pine.fbx`, `Assets/models/terrain_pine.vmdl`.
- Update documentation: `docs/assets/briefs/terrain_pine.md`, `docs/superpowers/specs/2026-05-13-terrain-pine-white-pine-design.md`.
...
- [ ] Add `scripts/terrain_pine_asset_pipeline.json` with `root_object` set to `TerrainPine_Root`, `target_fbx` set to `Assets/models/terrain_pine.fbx`, and existing bark/needle material remaps.
...
- [ ] Run `python scripts/asset_pipeline.py --config scripts/terrain_pine_asset_pipeline.json` to verify pine-only export selection and material remaps.
- [ ] Confirm `Assets/scenes/main.scene` still references `models/terrain_pine.vmdl` without scene edits.
